# Rubric: Local Service Boundary

| Criterion | Excellent | Passing | Needs Work |
| --- | --- | --- | --- |
| Contract correctness | Implements the required behavior for health metadata, request validation, error shape, advice refusal, and trace fields with deterministic, reviewable outputs | Required tests pass for the core behavior | Outputs are incomplete, ad hoc, or depend on hidden assumptions |
...
| FinAgent transfer | Explains how the work strengthens FinAgent or a practical AI system: FinAgent can expose an API-style contract without adding hosted infrastructure | Gives a plausible transfer note | Cannot connect the lesson to a real system outcome |
