...
Name the contract before editing code. For this lesson, focus on structured events, failure categories, summaries, and operational actions.
...
- transfer evidence: FinAgent can explain failures after a bad run instead of leaving vague logs
...
Use this hint entry for `build_log_event`. First read the function docstring and the nearest TODO, then compare the expected input and output shape in the tests.

### categorize_failure

Use this hint entry for `categorize_failure`. First read the function docstring and the nearest TODO, then compare the expected input and output shape in the tests.

### summarize_monitoring_events

Use this hint entry for `summarize_monitoring_events`. First read the function docstring and the nearest TODO, then compare the expected input and output shape in the tests.

### build_review_loop

Use this hint entry for `build_review_loop`. First read the function docstring and the nearest TODO, then compare the expected input and output shape in the tests.
