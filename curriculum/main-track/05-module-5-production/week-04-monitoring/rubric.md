# Rubric: Monitoring Review

| Criterion | Excellent | Passing | Needs Work |
| --- | --- | --- | --- |
| Contract correctness | Implements the required behavior for structured events, failure categories, summaries, and operational actions with deterministic, reviewable outputs | Required tests pass for the core behavior | Outputs are incomplete, ad hoc, or depend on hidden assumptions |
...
| FinAgent transfer | Explains how the work strengthens FinAgent or a practical AI system: FinAgent can explain failures after a bad run instead of leaving vague logs | Gives a plausible transfer note | Cannot connect the lesson to a real system outcome |
