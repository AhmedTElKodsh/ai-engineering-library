...
- package the app locally with a clean run command
...
| --- | --- | --- | --- |
| Week 1 | `week-01-golden-datasets` | Golden examples, edge cases, eval objective, and summary report | `python -m pytest curriculum/main-track/05-module-5-production/week-01-golden-datasets/tests -v` |
| Week 2 | `week-02-cicd` | Prompt/model/index version notes plus unit/eval smoke gate and CI-style command checklist | `python -m pytest curriculum/main-track/05-module-5-production/week-02-cicd/tests -v` |
| Week 3 | `week-03-fastapi` | Local service boundary with health and error responses | `python -m pytest curriculum/main-track/05-module-5-production/week-03-fastapi/tests -v` |
| Week 4 | `week-04-monitoring` | Structured logs, traces, failure categories, and review loop | `python -m pytest curriculum/main-track/05-module-5-production/week-04-monitoring/tests -v` |
| Week 5 | `week-05-optimization` | Cost, latency, caching, batching, and retry tradeoffs | `python -m pytest curriculum/main-track/05-module-5-production/week-05-optimization/tests -v` |

## Extended Concepts

Move to these only after the service boundary and release/eval gates are clear:

| Extended order | Folder | When to use it |
| ---: | --- | --- |
| 1 | `../../extended-concepts/03-production-depth/week-01-reproducible-package` | When a reviewer needs a stronger package/container run path than a clean local command. |
| 2 | `../../extended-concepts/03-production-depth/week-02-model-adaptation` | When eval evidence suggests prompt/RAG/tooling changes may not be enough and adaptation deserves analysis. |
