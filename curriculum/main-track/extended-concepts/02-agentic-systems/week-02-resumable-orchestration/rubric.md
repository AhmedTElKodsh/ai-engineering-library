# Rubric: Resumable Orchestration

| Criterion | Excellent | Passing | Needs Work |
| --- | --- | --- | --- |
| Contract correctness | Implements the required behavior for checkpointed steps, retry limits, error recording, and resume decisions with deterministic, reviewable outputs | Required tests pass for the core behavior | Outputs are incomplete, ad hoc, or depend on hidden assumptions |
...
| FinAgent transfer | Explains how the work strengthens FinAgent or a practical AI system: FinAgent can recover from interrupted or failed workflows without looping forever | Gives a plausible transfer note | Cannot connect the lesson to a real system outcome |
