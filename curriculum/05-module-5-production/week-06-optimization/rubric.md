# Rubric: Optimization Tradeoffs

| Criterion | Excellent | Passing | Needs Work |
| --- | --- | --- | --- |
| Contract correctness | Implements the required behavior for cost estimates, batching, cache policy, retry policy, and budget recommendations with deterministic, reviewable outputs | Required tests pass for the core behavior | Outputs are incomplete, ad hoc, or depend on hidden assumptions |
...
| FinAgent transfer | Explains how the work strengthens FinAgent or a practical AI system: FinAgent optimizations stay tied to measured reliability, cost, and latency | Gives a plausible transfer note | Cannot connect the lesson to a real system outcome |
