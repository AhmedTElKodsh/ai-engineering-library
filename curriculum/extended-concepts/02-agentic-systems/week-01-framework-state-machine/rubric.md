# Rubric: Framework State Machine

| Criterion | Excellent | Passing | Needs Work |
| --- | --- | --- | --- |
| Contract correctness | Implements the required behavior for immutable state, explicit transitions, completed steps, and current position with deterministic, reviewable outputs | Required tests pass for the core behavior | Outputs are incomplete, ad hoc, or depend on hidden assumptions |
...
| FinAgent transfer | Explains how the work strengthens FinAgent or a practical AI system: FinAgent can move toward framework orchestration only after the state is inspectable | Gives a plausible transfer note | Cannot connect the lesson to a real system outcome |
