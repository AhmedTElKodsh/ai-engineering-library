...
**🚩 FLAGSHIP PROJECT #1 STARTS WEEK 5** — You will build one RAG system across the curriculum, evolving it through three named stages: RAG Reference Implementation (v1, Week 5) → Production-Style RAG System (v2, +auth) → Enterprise RAG Platform (v3, +observability & multi-tenancy). Each rename is earned, not assumed.
...
|----------|-------------|-------|
| **Flagship #1: RAG System (3-Stage Evolution)** | v1 — RAG Reference Implementation: hybrid search, reranking, eval pipeline, cloud-deployed. v2 — Production-Style RAG: adds auth, RBAC, audit logging. v3 — Enterprise RAG Platform: adds structured observability, multi-tenancy, `/health` + `/ready` endpoints, p95 latency tracking. | RAG architecture, iterative system hardening, production engineering, evaluation thinking, observability design |
