...
Provide your reflection. On the LAST LINE, write exactly one word: CONTINUE or COMPLETE."""

        reflection = await self.llm.complete(prompt)
        self._context.add_reflection(reflection)

        # Parse decision from the last line only to avoid substring false-positives
        # (e.g. "INCOMPLETE", "NOT COMPLETE" would wrongly trigger the old check)
        last_line = reflection.strip().split('\n')[-1].strip().upper()
        should_complete = last_line == "COMPLETE" or not self._context.should_continue()
